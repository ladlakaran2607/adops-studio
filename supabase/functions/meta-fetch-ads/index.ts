...
serve(async (req) => {
  ...
  try {
    ...
    let url: string | null = metaUrl(
      `act_${account_id}/ads?fields=id,name,status,creative{id,asset_feed_spec,object_story_spec,effective_object_story_id}&limit=100&access_token=${encodeURIComponent(accessToken)}`
    );
